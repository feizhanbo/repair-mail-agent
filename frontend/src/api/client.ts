...
function friendlyServerMessage(status?: number, code?: string): string {
  const messages: Record<string, string> = {
    ...
    SN_ASSET_NOT_FOUND: 'SN 资料不存在或已删除',
    SN_ALREADY_EXISTS: 'SN 已存在，请检查编号',
    SN_IDENTITY_IN_USE: '该 SN 已被业务引用，不能修改 SN 编号',
    SN_ASSET_IN_USE: '该 SN 已被业务引用，请改为无效状态',
    BOARD_CARD_NOT_FOUND: '板卡规则不存在或已删除',
    BOARD_CARD_IN_USE: '该板卡规则已被工单引用，请改为停用',
    CUSTOMER_POLICY_IN_USE: '该客户政策已被工单引用，请改为停用',
    POLICY_CURRENCY_INVALID: '币种仅支持 RMB 或 USD',
    SN_SYNC_COLUMN_MAP_INVALID: 'SN 字段映射包含不允许的字段或非法列名',
  };
...
export const api = {
  ...
  linkedTickets: (id: number) => getData<LinkedTicket[]>(`/emails/${id}/linked-tickets`),
  emailDeletePreview: (id: number) => getData<DeletePreview>(`/emails/${id}/delete-preview`),
  deleteEmail: (id: number, confirmationToken: string) =>
    apiClient.delete<ApiResponse<Record<string, unknown>>, ApiResponse<Record<string, unknown>>>(`/emails/${id}`, { params: { confirmation_token: confirmationToken } }).then((response) => response.data),
  ...
  ticketDeletePreview: (id: number) => getData<DeletePreview>(`/tickets/${id}/delete-preview`),
  deleteTicket: (id: number, confirmationToken: string) =>
    apiClient.delete<ApiResponse<Record<string, unknown>>, ApiResponse<Record<string, unknown>>>(`/tickets/${id}`, { params: { confirmation_token: confirmationToken } }).then((response) => response.data),
  ...
  updateSnAsset: (id: number, body: Record<string, unknown>) => patchData<SnAsset>(`/master-data/sn-assets/${id}`, body),
  snAssetDeletePreview: (id: number) => getData<DeletePreview>(`/master-data/sn-assets/${id}/delete-preview`),
  deleteSnAsset: (id: number) => apiClient.delete<ApiResponse<Record<string, unknown>>, ApiResponse<Record<string, unknown>>>(`/master-data/sn-assets/${id}`).then((response) => response.data),
  ...
  updateBoardCard: (id: number, body: Record<string, unknown>) => patchData<BoardCard>(`/master-data/board-cards/${id}`, body),
  boardCardDeletePreview: (id: number) => getData<DeletePreview>(`/master-data/board-cards/${id}/delete-preview`),
  deleteBoardCard: (id: number) => apiClient.delete<ApiResponse<Record<string, unknown>>, ApiResponse<Record<string, unknown>>>(`/master-data/board-cards/${id}`).then((response) => response.data),
  ...
  customerPolicyDeletePreview: (id: number) => getData<DeletePreview>(`/master-data/customer-policies/${id}/delete-preview`),
  deleteCustomerPolicy: (id: number) => apiClient.delete<ApiResponse<Record<string, unknown>>, ApiResponse<Record<string, unknown>>>(`/master-data/customer-policies/${id}`).then((response) => response.data),
  snSyncConfig: () => getData<SnSyncConfig>('/system/sn-sync/config'),
  updateSnSyncConfig: (body: Partial<SnSyncConfig>) => patchData<SnSyncConfig>('/system/sn-sync/config', body),
  startSnSync: () => postData<SapSnSyncBatch>('/system/sn-sync'),
  latestSnSync: () => getData<SapSnSyncBatch | null>('/system/sn-sync/latest'),
