...
REQUIRED_REVISION = "k8f3a4b5c6d7"
...
async def audit(expected_database: str, backup: Path | None) -> dict[str, Any]:
    ...
    async with AsyncSessionLocal() as session:
        ...
        close_transitions = await _rows(
            session,
            text(
                "SELECT from_status_code, trigger_event, enabled, require_manual "
                "FROM workflow_transitions WHERE to_status_code='closed' "
                "ORDER BY from_status_code, trigger_event"
            ),
        )
    ...
    enabled_close_routes = [
        (row["from_status_code"], row["trigger_event"])
        for row in close_transitions
        if bool(row["enabled"])
    ]
...
def _audit_failures(report: dict[str, Any], *, require_backup: bool) -> list[str]:
    failures: list[str] = []
    database = report["database"]
    schema = report["schema"]
    if not database["matches_expected"]:
        failures.append("DATABASE_NAME_MISMATCH")
...
def main() -> int:
    ...
    failures = _audit_failures(report, require_backup=args.backup is not None)
